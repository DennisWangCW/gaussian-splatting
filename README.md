# Environment Configurations for Running Gaussian-Splatting in Containers


## Install Docker and Nvidia-Docker

To install docker on your host machine, follow the steps at [Docker Installation Guide](https://docs.docker.com/engine/install/ubuntu/).

To install nvidia-docker, follow the steps at [Nvidia-Docker Installation Guide](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html).
...
__Note__: Refer to [Docker Support1 ](https://github.com/graphdeco-inria/gaussian-splatting/blob/75c0ce9e4461fcd49c7d1488767ebd5a9e3821d8/Dockerfile) and [Docker Support2](https://github.com/graphdeco-inria/gaussian-splatting/blob/1acf7a3900ece5d23d7621f0b8090816301a5e45/docker/Dockerfile) for more details.

Before entering the container, execute
```bash
xhost +
```
on your host machine to allow any X client access your host X service for the display purpose. 

## Install Some Dependencies

```bash
apt-get install -y curl wget vim locate git
```

## Build Ceres Solver (Optionally)

To build Ceres Solver, follow these steps:

```bash
apt-get install -y libeigen3-dev libgoogle-glog-dev 
git clone https://ceres-solver.googlesource.com/ceres-solver
cd ceres-solver
mkdir build
cd build
cmake .. -DBUILD_TESTING=OFF -DBUILD_EXAMPLES=OFF -DCMAKE_CUDA_ARCHITECTURES="60;70;80;86" #-DUSE_CUDA=OFF 
make -j8
make install
```

__Note__: You may encounter a warning regarding conflicting targets for glog [GLOG CONFLICT](https://github.com/ceres-solver/ceres-solver/issues/1049). To fix this, edit `/usr/local/lib/cmake/Ceres/FindGlog.cmake` as follows:
...
## Build COLMAP (Optionally, in case you rebuild colmap)

To build COLMAP, execute the following commands:

```bash
apt-get install -y libgl1-mesa-dev libcgal-dev libboost-all-dev libflann-dev qt5-default libmetis-dev libglew-dev libfreeimage-dev 
...
mkdir build
cd build
cmake .. -DCMAKE_CUDA_ARCHITECTURES="60;70;80;86" #-DCUDA_ENABLED=OFF 
make -j8
make install
```

__Note__: During the build process, you may encounter a warning related to conflicting runtime search paths. To address this issue, add the following lines to `src/colmap/exe/CMakeLists.txt`:

```cmake
set_target_properties(colmap_main PROPERTIES
    BUILD_WITH_INSTALL_RPATH TRUE
    INSTALL_RPATH "/usr/lib/x86_64-linux-gnu:/opt/conda/lib")
```

Remember to add the following line to the bashrc file:

```bash
export PATH=$PATH:/workspace/colmap/build/src/colmap/exe
```

## Visualize Trained Models
You may encounter an error related to GLFW [GLFW ERROR](https://blog.csdn.net/weixin_44153180/article/details/131721970) when executing SIBR_remotegaussian_app command. Set the the following environment variable can solve the problem.

```bash
export MESA_GL_VERSION_OVERRIDE=4.5
```
To view point clouds, install the point cloud library by:

```bash
apt-get install -y libpcl-dev pcl-tools
```
